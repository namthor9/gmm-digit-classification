# -*- coding: utf-8 -*-
"""
Created on Tue Nov  8 11:39:03 2022
...
for digit_list in full_digits_data:
    ...
    axes[x][n % 5].tick_params(labelsize=8)
    axes[x][n % 5].ticklabel_format(axis='y',style='scientific')
    axes[x][n % 5].plot(np.arange(1,10),ss)   
    axes[x][n % 5].set_xticks([1,2,3,4,5,6,7,8,9])
    axes[x][n % 5].set_xticklabels(["1","2","3","4","5","6","7","8","9"])
    axes[x][n % 5].locator_params(tight=True, nbins=4)
    n+=1        
plt.subplots_tool() 
#fig.tight_layout()
exit()
    ...
    for i in range(660):
          for k in range(len(digit_list[i][0])):
              ...
              for j in range(n_dims):
                  new_point.append(digit_list[i][j][k])
              unpacked.append(new_point)          
    kmeans = KMeans(n_clusters=4).fit(unpacked)
    gmm = GaussianMixture(n_components=4,covariance_type='full').fit(unpacked)
    gmm_list.append(gmm)
    g1 = []
    g2 = []
    g3 = []
...
plt.title("KMeans Confusion Matrix, n_clusters={}, n_features={}".format(4,n_dims))
...
# plt.xlabel("True Digit")
# plt.ylabel("Predicted Digit")
plt.title("GMM Confusion Matrix, n_clusters={}, n_features={}".format(4,n_dims))
